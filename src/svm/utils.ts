...
import {
    decodeNameRecordHeader,
    fetchMaybeNameRecordHeader,
    fetchNameRecordHeader,
...
export type ProgramAccountsLoaderValue = ReturnType<
    GetProgramAccountsApi['getProgramAccounts']
>;
...
export async function getNameOwner(
    ...
    const nameAccount = await fetchMaybeNameRecordHeader(rpc, nameAccountKey);
...
export async function findOwnedNameAccountsForUser(
    ...
    const accounts = await rpc
        .getProgramAccounts(ANS_PROGRAM_ID, {
            encoding: 'base64',
            filters: filters,
...
> {
    ...
    const accounts = await rpc
        .getProgramAccounts(TLD_HOUSE_PROGRAM_ID, {
            filters: filters,
            encoding: 'base64',
        })
        .send();
...
export function getParentAccountFromTldHouseAccountInfo(
    ...
    const data = base64Codec.encode(tldHouseData.account.data[0]);
    const parentAccountBuffer = data.slice(
        parentAccountStart,
...
export async function getMintOwner(
    ...
    try {
        ...
        const largestAccounts = await rpc
            .getTokenLargestAccounts(nftRecordData.data.nftMintAccount)
            .send();
...
export async function findTldHouse(
    tldString: string,
): Promise<readonly [Address, number]> {
...
export async function findNameHouse(
    tldHouse: Address,
): Promise<readonly [Address, number]> {
    return await getProgramDerivedAddress({
        programAddress: NAME_HOUSE_PROGRAM_ID,
...
export async function performReverseLookupBatched(
    ...
    const promises = nameAccounts.map(async nameAccount => {
        const reverseLookupHashedName = await getHashedName(nameAccount);
    ...
    const reverseLookupAccountsDecoded = reverseLookupAccountInfos.map(
        accountInfo => decodeNameRecordHeader(accountInfo),
    );

    return reverseLookupAccountsDecoded.map(reverseLookupAccountInfo => {
        if (!reverseLookupAccountInfo.exists) return;
        const domain = reverseLookupAccountInfo.data.data
            ?.subarray(
                getNameRecordHeaderSize(),
...
export const getOwnerNfts = async (
    ...
    const { value: splAccounts } = await rpc
    ...
    const processBatch = async (batch: string[]) => {
        ...
        for (const nftMetadata of nftsMetadata) {
            if (nftMetadata.exists) {
                ...
                if (isVerified && verifiedCreatorAddress === expectedCreator) {
                    const domainName = utf8Codec
                        .decode(nftMetadata.data.subarray(66, 101))
                        .replace(/\u0000/g, '');
...
export async function getMintAccountFromDomainTld(
    ...
    return await getDomainMintAccountKey(rpc, domainAccountKey, tldHouse);
}
...
export const getMultipleMainDomainsChecked = async (
    rpc: Rpc<
        GetAccountInfoApi & GetTokenLargestAccountsApi & GetMultipleAccountsApi
    >,
    pubkeys: string[],
): Promise<
    {
...
> => {
    const mainDomainKeys = await Promise.all(
        pubkeys.map(async pubkey => await findMainDomain(pubkey as Address)),
    );

    // fetch main domain accounts
    const mainDomainAccounts = await fetchEncodedAccounts(
        rpc,
        mainDomainKeys.map(key => key[0]),
    );
    ...
    const mainDomainWithNameAccounts = pubkeys.map((pubkey, index) => {
        ...
        if (mainDomainAccount.exists) {
            ...
            return {
                ...
                nameAccount,
                mainDomain:
                    mainDomainData.data.domain + mainDomainData.data.tld,
            };
        }
        return {
            pubkey: pubkey.toString(),
    ...
    const nameAccountsInfo = await fetchEncodedAccounts(
        rpc,
        filteredNameAccounts,
    );
    ...
    const result = await Promise.all(
        mainDomainWithNameAccounts.map(async (item, index) => {
            ...
            const [nftRecordKey] = await findNftRecord(
                nameAccount,
                nameHouseKey,
            );
            ...
            const expiresAtIsValid =
                nameAccountData.data.expiresAt.getTime() != 0;
            if (
                expiresAtIsValid &&
