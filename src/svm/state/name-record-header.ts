...
export function getNameRecordHeaderDecoder(): Decoder<NameRecordHeader> {
    ...
    return transformDecoder(
        ...
        (decoded, bytes, offset) => {
            ...
            const expiresAtMs =
                decoded.expiresAt === 0n ? 0 : Number(decoded.expiresAt) * 1000;
            const nowMs = Date.now();
            const isValid =
                decoded.expiresAt === 0n
                    ? true
                    : expiresAtMs + GRACE_PERIOD_MS > nowMs;
            const expiresAt = new Date(expiresAtMs);
            const createdAtMs =
                decoded.createdAt === 0n ? 0 : Number(decoded.createdAt) * 1000;
...
// cannot be combined since data has to be altered to add validity.
