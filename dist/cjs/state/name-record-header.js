...
class NameRecordHeader {
    constructor(obj) {
        ...
        this.expiresAt = new Date(new borsh_1.BinaryReader(Buffer.from(obj.expiresAt)).readU64().toNumber() *
            1000);
        this.isValid =
            new borsh_1.BinaryReader(Buffer.from(obj.expiresAt))
                .readU64()
                .toNumber() === 0
                ? true
    ...
    static fromAccountInfo(nameAccountAccountInfo) {
        var _a;
        const res = (0, borsh_1.deserializeUnchecked)(this.schema, NameRecordHeader, nameAccountAccountInfo.data);
